...
# PDCLab
#
# cloud-init/pdclab-veloxchem-dev.sh - Cloud-init script for a Rocky 9 PDCLab VM with VeloxChem build environment  
...
PDCLAB_SRC_URL="${PDCLAB_BASE_URL}/v0.1.4"
...
curl ${PDCLAB_BASE_URL}/bin/build-vlx.sh > /tmp/build-vlx.sh
...
for i in "${!VLX_BRANCHES[@]}"; do
    ...
    su - ${LAB_USERS[0]} -c "labexec condaenv ${env_dir} conda install -y -c conda-forge ipykernel xtb-python ipywidgets py3Dmol k3d"

    echo "configuring compute kernel from ${env_dir} for user ${LAB_USERS[0]}..."
    su - ${LAB_USERS[0]} -c "labexec condaenv ${env_dir} python -m ipykernel install --user --name 'VeloxChem-${branch}'"
done
