...
# KCSCLab
#
# cloud-init/veloxchem-dev.sh - cloud-init script for a Rocky 9 KCSCLab VM with VeloxChem build environment
...
# KCSCLab content base URL
KCSCLAB_BASE_URL="https://s3.dc.pdc.kth.se/swift/v1/AUTH_b617f6725c17476383046e41a7c776a2/public/kcsclab"

# KCSCLab deployment URL
KCSCLAB_SRC_URL="${KCSCLAB_BASE_URL}/v0.2.1"

# KCSCLab conda envs URL
KCSCLAB_CONDA_URL="${KCSCLAB_BASE_URL}/envs/conda"
...
# kcsclab runtime environment
for file_path in /usr/local/libexec/kcsclab.sif{,.sha256} /etc/{profile.d/kcsclab.sh,sysconfig/kcsclab,systemd/system/kcsclab\@.service}; do
    curl ${KCSCLAB_SRC_URL}${file_path} >${file_path}
done

# veloxchem build script
curl ${KCSCLAB_BASE_URL}/bin/build-vlx.sh > /tmp/build-vlx.sh
chmod a+x /tmp/build-vlx.sh

# ASYNC: verify kcsclab image checksum
(cd /usr/local/libexec; sha256sum -c kcsclab.sif.sha256)

# enable kcsclab
chmod a+x /usr/local/libexec/kcsclab.sif
...
source /etc/profile.d/kcsclab.sh
...
sed -e "s/^RUNTIME_OPTS=.*/RUNTIME_OPTS=\"--writable-tmpfs --mount type=bind,src=$(echo ${MOUNT_DIR} | sed 's#\/#\\\/#g'),dst=$(echo ${MOUNT_DIR} | sed 's#\/#\\\/#g')\"/" -i /etc/sysconfig/kcsclab
...
# ============================
# build VeloxChem environments
# ============================

su - ${LAB_USERS[0]} -c "mkdir -p ${LAB_ENV_BASE}"

for i in "${!VLX_BRANCHES[@]}"; do
    branch="${VLX_BRANCHES[$i]}"
    env_dir="${LAB_ENV_BASE}/${branch}"

    echo "building veloxchem from branch ${branch} into ${LAB_ENV_BASE}..."
    su - ${LAB_USERS[0]} -c "labexec /tmp/build-vlx.sh -b ${branch} -e ${env_dir}"

    echo "installing packages into ${env_dir}..."
    su - ${LAB_USERS[0]} -c "labexec conda-run ${env_dir} conda install -y -c conda-forge ipykernel ipywidgets py3Dmol k3d"

    echo "configuring compute kernel from ${env_dir} for user ${LAB_USERS[0]}..."
    su - ${LAB_USERS[0]} -c "labexec conda-run ${env_dir} python -m ipykernel install --user --name 'VeloxChem-${branch}'"
done


# =================================================================
# HTTP/SSL proxy build (TODO: move all into kcsclab-proxy container)
# =================================================================
...
systemctl enable kcsclab@${LAB_USERS[0]}.service
...
systemctl start kcsclab@${LAB_USERS[0]}.service &
