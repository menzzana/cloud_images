...
# KCSCLab
#
# cloud-init/envs-rocky.sh - cloud-init script for a Rocky 9 KCSCLab VM with example environments
...
readarray conda_envs_raw < <(curl -s ${KCSCLAB_CONDA_URL}/MANIFEST | egrep "^[a-zA-Z]")
...
function build_conda_env() {
    ...
    curl ${KCSCLAB_CONDA_URL}/${env_file} >${env_file_path}

    su - ${user_name} -c "labexec conda-cmd env create -f ${env_file_path} -p ${env_dir_path}"
    su - ${user_name} -c "labexec conda-run ${env_dir_path} python -m ipykernel install --user --name=${env_name}"
}
